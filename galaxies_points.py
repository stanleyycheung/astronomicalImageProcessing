...
def galaxyDistribution(galaxies_points, img):
    for counter in range(0, 350):
        ...
        #     print(x_diff, y_diff)
        #     fig, ax = plt.subplots()
        #     plt.imshow(img[x_min:x_max, y_min:y_max], origin='upper', norm=LogNorm())
        #     fig, ax = plt.subplots()
        #     plt.imshow(digitalMap[x_min:x_max, y_min:y_max], origin='upper')
        #     np.delete(galaxies_points, counter)
        #     continue
        ...
        if len(x_peak) > 1 or len(y_peak) > 1:
            ...
            fig, ax = plt.subplots()
            plt.imshow(digitalMap[x_min:x_max+1, y_min:y_max+1], origin='upper')


if __name__ == '__main__':
    img = np.load('realmaskedData.npy')
    digitalMap = np.load('digitalMap.npy')
